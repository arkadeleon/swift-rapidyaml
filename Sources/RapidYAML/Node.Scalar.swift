...
extension Node {
    ...
    public struct Scalar {
        ...
        ///
        /// - note: Yams declares this `weak`, which leaves it `nil` for every node the caller gets
        ///         back from `compose`, since nothing else keeps the `Anchor` alive once the parser
        ///         is gone. It is held strongly here so that the anchor survives composition.
