...
//  RapidYAMLTests
//
//  Created by Leon Li on 2026/8/4.
//
...
final class PerformanceTests: XCTestCase, @unchecked Sendable {
    ...
    func parseSourceKittenIssue289UsingLoad(yaml: String, encoding: Parser.Encoding) {
        ...
        do {
            guard let object = try RapidYAML.load(yaml: yaml, .default, .default, encoding) as? [String: Any],
                ...
                let sources = moduleCommand["sources"] as? [String] else {
                    XCTFail("Invalid result form RapidYAML.load()")
                    return
            }
    ...
    func parseSourceKittenIssue289UsingCompose(yaml: String, encoding: Parser.Encoding) {
        let spmName = "SourceKittenFramework"
        do {
            guard let node = try RapidYAML.compose(yaml: yaml, .default, .default, encoding),
                ...
                let sources = moduleCommand["sources"]?.array(of: String.self) else {
                    XCTFail("Invalid result form RapidYAML.compose()")
                    return
            }
            XCTAssertEqual(imports, self.expectedImports)
            XCTAssertEqual(otherArguments, self.expectedOtherArguments)
    ...
    func parseSourceKittenIssue289UsingSwiftDecodable(yaml: String, encoding: Parser.Encoding) {
        let spmName = "SourceKittenFramework"
        do {
            let decoder = YAMLDecoder(encoding: encoding.swiftStringEncoding)
            guard let manifest: Manifest = try decoder.decode(from: yaml),
                ...
                let sources = command.sources else {
                    XCTFail("Invalid result form YAMLDecoder.decode()")
                    return
            }
            XCTAssertEqual(imports, self.expectedImports)
            XCTAssertEqual(otherArguments, self.expectedOtherArguments)
