...
@discardableResult
func AssertYAMLEqual(_ lhs: Any?, _ rhs: Any?,
...
func AssertYAMLEqualType(_ actual: Any?, _ expected: Any?, path: [String] = []) {
    // swiftlint:disable:previous identifier_name
    ...
    case let (aDict as [String: Any], eDict as [String: Any]):
        ...
        for key in eDict.keys {
            ...
            AssertYAMLEqualType(aValue, eValue, path: path + [key])
        }
    ...
    case let (aArray as [Any], eArray as [Any]):
        ...
        for index in 0..<eArray.count {
            AssertYAMLEqualType(aArray[index], eArray[index], path: path + ["[\(index)]"])
        }
