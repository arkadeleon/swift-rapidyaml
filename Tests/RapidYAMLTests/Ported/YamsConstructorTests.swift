...
final class ConstructorTests: XCTestCase, @unchecked Sendable { // swiftlint:disable:this type_body_length
    ...
    func testBinary() throws {
        let example = """
            ...
            """
        let objects = try RapidYAML.load(yaml: example)
        ...
        AssertYAMLEqual(objects, expected)
    }
    ...
    func testMap() throws {
        let objects = try RapidYAML.load(yaml: Self._mapExample)
    ...
    func testNSMutableMap() throws {
        ...
        AssertYAMLEqualType(objects, expected)
        AssertYAMLEqual(objects, expected)
    }
    ...
    func testNull() throws {
        let example = """
            ...
            """
        let objects = Array(try RapidYAML.load_all(yaml: example))
        let expected: [Any] = [
            NSNull(),
            [
                "empty": NSNull(),
    ...
    func testSet() throws {
        // The block half of Yams' example — `? Mark McGwire` indented under a key, then a sibling
        // key at column 0 — is rejected by rapidyaml as invalid indentation, where libyaml accepts
        // it. See `explicitKeyBlocksCannotBeFollowedBySiblings` for the narrowed case. The flow
        // form, and `? key` blocks that are not followed by a sibling, both work.
        let example = """
            # Flow style
    ...
    func testSeq() throws {
        let objects = try RapidYAML.load(yaml: Self._seqExample)
        let expected: [String: Any] = [
            "Block style": [
                "Mercury",
                "Venus",
    ...
    func testValue() throws {
        let example = """
            ...
            """
        let nodes = Array(try RapidYAML.compose_all(yaml: example))
        let expected: [Node] = [
            [
        ...
        AssertYAMLEqual(nodes, expected)
